...
# Load environment spec dynamically
ENV_SPEC = json.loads((ROOT / "environment_spec.json").read_text(encoding="utf-8"))
ZONES = ENV_SPEC["zones"]
DEPENDENCY_BY_ZONE = ENV_SPEC["dependencies"]
NON_IDEAL_CONDITIONS = ["low_resource", "bad_quality"]
ALL_CONDITIONS = ["ideal", "low_resource", "bad_quality"]
INPUT_ACTION_BY_ZONE = ENV_SPEC["input_action_by_zone"]
PRIMITIVE_TEMPLATE_BY_ZONE = {
    zone: templates
    for zone, templates in ENV_SPEC["primitive_template_by_zone"].items()
}
...
def generate_symbolic_rows(mode: str) -> list[dict[str, str]]:
    ...
    synthetic_id = 1
    max_goal_level = len(ZONES) - 1  # 4 for 5 zones
        ...
        for target_zone in ZONES:
            ...
            for active_pattern in itertools.product(NON_IDEAL_CONDITIONS, repeat=goal_level):
                ...
                for free_assignment in free_assignments:
                    ...
                    row = {
                        "source_mission_id": f"efghi_{mode}_{synthetic_id:04d}",
                    ...
                    for z in ZONES:
                        row[f"{z}_condition"] = zone_conditions[z]
                    rows.append(row)
                    synthetic_id += 1
    return rows
    ...
    for goal_level in sorted(rows_by_level.keys()):
        ...
        level_rows.sort(
            key=lambda row: (
                row["target_zone"],
                row["template_id"],
                *(row[f"{z}_condition"] for z in ZONES),
